...
OUTPUT_HTML = "preview_tese-runpdf.html"
# ─────────────────────────────────────────────────────────────────────────────
...
def find_meta(project_root: Path) -> dict:
    ...
    def get(key):
        ...
        # Se achar marcações vazias do template, descarta para usar o fallback
        return "" if re.match(r'^.*NOME DO ALUNO.*$', v) or re.match(r'^.*TÍTULO DO TRABALHO.*$', v) or re.match(r'^<.*>$', v) else v

    # Fallbacks inteligentes para o seu projeto específico
    return {
...
SKIP_ENVS = {'figure','table','verbatim','lstlisting','tikzpicture',
...
class TexParser:
    def __init__(self, path: Path, is_pre=False, ch_start=0):
        self.path = path
        self.is_pre = is_pre
        ...
        for line in raw.split('\n'):
            ...
            if t is not None:
                ...
                if self.is_pre:
                    ...
                    self.ch_count += 1
                    out.append(f'<h1 class="chapter">{self._esc(t)}</h1>')
                continue

            t = try_title(r'\\section\*?\{([^}]+)\}')
            if t is not None:
                flush(); out.append(f'<h2>{self._esc(t)}</h2>'); continue

            t = try_title(r'\\subsection\*?\{([^}]+)\}')
            if t is not None:
                flush(); out.append(f'<h3>{self._esc(t)}</h3>'); continue

            if re.match(r'\\(usepackage|documentclass|newcommand|renewcommand|'
...
# CSS Ajustado com precisão para o PPGC_UFF.cls
CSS = """
...
/* Capa - Exatamente igual ao original UFF */
.capa{display:flex;flex-direction:column;align-items:center;
      justify-content:space-between;height:100%;
      text-align:center;text-transform:uppercase}
.capa-inst{font-weight:bold;font-size:12pt}
.capa-autor{font-weight:bold;font-size:12pt;margin-top:auto}
.capa-titulo{font-weight:bold;font-size:12pt;margin:35mm 0}
.capa-local{font-weight:bold;font-size:12pt;margin-top:auto}

/* Folha de rosto */
.rosto{display:flex;flex-direction:column;align-items:center;
       height:100%;text-align:center;}
.rosto-autor{font-weight:bold;font-size:12pt;text-transform:uppercase}
.rosto-titulo{font-weight:bold;font-size:12pt;text-transform:uppercase;margin:35mm 0}
.rosto-req-container{width:100%; display:flex; justify-content:flex-end;}
.rosto-req{width:50%;text-align:justify;font-size:10pt;line-height:1.1;text-indent:0!important;}
.rosto-orientadores{width:100%; text-align:left; font-size:10pt; line-height:1.1; margin-top:10mm; margin-left: 50%;}
.rosto-local{font-weight:bold;font-size:12pt;margin-top:auto;text-transform:uppercase}

/* Títulos (Ajustado para Title Case como no PDF original) */
h1.titulo-pre{font-size:14pt;font-weight:bold;text-align:center;margin-bottom:24pt;page-break-before:always}

/* Contadores */
body{counter-reset:chapter}
h1.chapter{font-size:14pt;font-weight:bold;text-align:left;
           margin-bottom:24pt;page-break-before:always;
...
h4{font-size:12pt;font-weight:bold;font-style:italic;margin-top:18pt;margin-bottom:12pt;
   counter-increment:subsubsection}
h4::before{content:counter(chapter)"."counter(section)"."counter(subsection)"."counter(subsubsection)" "}
...
/* Aviso */
.aviso{position:fixed;bottom:15px;right:15px;background:#fff3cd;border:1px solid #ffc107;
       border-radius:6px;padding:10px 14px;font-family:sans-serif;font-size:11px;
       z-index:1000;box-shadow:0 2px 6px rgba(0,0,0,.2);max-width:260px;line-height:1.4}
"""
...
def generate(project_root: Path):
    ...
    pages = [build_capa(meta), build_rosto(meta)]
